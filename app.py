    ...
    def calculate_scores(self):
        ...
        if self.type == "Everything Everywhere all at once":
            for player_id, claims in self.players_folds.items():
                for claim in claims:
                    self.scores[player_id] += types["Folds"]
            self.scores[self.first_fold] += types["First and Last"]
            self.scores[self.last_fold] += types["First and Last"]
                ...
                for player_id, claims in self.players_folds.items():
                    for claim in claims:
                        for card in claim:
                            ...
                            if type_round in types and card in types[type_round]:
                                self.scores[player_id] += types[type_round][card]
